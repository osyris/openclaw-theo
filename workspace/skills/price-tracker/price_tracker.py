...
и извлечения цены спрятана внутри. По умолчанию цена берётся ДЕТЕРМИНИРОВАННО
(Shopify JSON / JSON-LD / OpenGraph-meta / микроданные) — модель не участвует.
Исключение — товары с заданным `extract_prompt` (команда `prompt`): для трудных
сайтов их цену извлекает модель (через одноразовую изолированную сессию), с
проверкой на вменяемость; в отчёте такие цены помечены «*».
...
Команды:
  ...
  prompt <id> "<подсказка>"    Включить извлечение цены моделью для трудного сайта
                               (подсказка — что именно искать). `prompt <id> --clear`
                               возвращает товар на детерминированное извлечение.
...
# Where `openclaw agent --session-id <id>` drops throwaway transcripts. We clean
# our own after each call; override with PT_SESSIONS_DIR if the layout changes.
SESSIONS_DIR = os.environ.get(
    "PT_SESSIONS_DIR", "/data/.openclaw/agents/main/sessions")
...
# Used only for items that carry an explicit `extract_prompt`. The fetch and the
# HTML→text cleanup are deterministic; only the final pick is the model's, and it
# runs in a throwaway, isolated openclaw session (the main chat is never touched).
MODEL_EXTRACT_INSTR = (
    'Найди ТЕКУЩУЮ цену этого товара — ту, что покупатель платит сейчас (если есть '
    'скидка, бери цену со скидкой, а не зачёркнутую старую). Игнорируй цену доставки, '
    'цену за единицу/грамм и цены других товаров. Ответь СТРОГО одним JSON-объектом '
    'без пояснений и текста вокруг: {"price": <число или null>, "currency": "<ISO, '
    'напр. EUR>"}.')

def html_to_text(body, cap=14000):
    """Strip rendered HTML to plain text deterministically (no model involved)."""
    if not body:
        return ""
    s = re.sub(r"(?is)<(script|style|noscript|svg|template)\b.*?</\1>", " ", body)
    s = re.sub(r"(?s)<!--.*?-->", " ", s)
    s = re.sub(r"(?s)<[^>]+>", " ", s)
...
def sweep_throwaway_sessions(max_age_s=7200):
    """Best-effort cleanup of orphaned throwaway transcripts left by crashes."""
    try:
        for f in glob.glob(os.path.join(SESSIONS_DIR, "pricetrack-*.jsonl")):
            try:
...
def extract_via_model(body, url, prompt, last_price=None):
    """Extract a price with the model, for sites that defeat structured parsing.

    Runs one turn in a fresh, isolated openclaw session (unique --session-id, no
    --deliver → clean stdout, main chat untouched), then deletes that throwaway
    transcript. Guards against garbage (≤0, or wildly off the last price) but NOT
    against legitimate discounts. Returns (price, currency) or None.
    """
    text = html_to_text(body)
    if not text:
        return None
    sid = "pricetrack-" + uuid.uuid4().hex
    msg = prompt.strip() + "\n\n" + MODEL_EXTRACT_INSTR + "\n\nТЕКСТ СТРАНИЦЫ:\n" + text
    rc, out, _ = oc(["agent", "--session-id", sid, "--thinking", "off",
                     "--timeout", "60", "--message", msg], timeout=90)
    try:                                   # always drop the throwaway transcript
        os.remove(os.path.join(SESSIONS_DIR, sid + ".jsonl"))
    except OSError:
        pass
    if rc != 0 or not out.strip():
        return None
    data = _loads_loose(out.strip())
    if not isinstance(data, dict):
        return None
    price = parse_price(data.get("price"))
    if price is None:
        return None
    cur = (str(data.get("currency") or "EUR").upper()[:3]) or "EUR"
    # plausibility guard: reject only garbage, never a real discount.
    if last_price and (price < last_price / 10 or price > last_price * 10):
        return None
    return price, cur
...
def resolve(item, cfg):
    ...
    Items with a non-empty `extract_prompt` are resolved by the model (rendered
    page → openclaw agent); everything else stays fully deterministic.
    """
    url = item["url"]
    prompt = (item.get("extract_prompt") or "").strip()
    if prompt:
        body = t_chromium(url) or t_cffi(url) or t_plain(url)
        if not body:
            return None
        r = extract_via_model(body, url, prompt, item.get("last_price"))
        return (r[0], r[1], "model") if r else None
...
def build_text(rows):
    ...
    if has_model:
        lines.append("")
        lines.append("* цена распознана моделью со страницы")
    return "\n".join(lines)
...
def cmd_prompt(args):
    # prompt <id> <текст подсказки...>   |   prompt <id> --clear
    if len(args) < 2:
        return fail(E_INPUT, "usage",
                    'prompt <id> "<подсказка>"  |  prompt <id> --clear')
    iid = args[0]
    cfg = load_config()
    for i in cfg.get("items", []):
        if i.get("id") == iid:
            if args[1] == "--clear":
                i.pop("extract_prompt", None)
                save_config(cfg)
                print("очищен extract_prompt у %s — снова детерминированное извлечение" % iid)
                return OK
            i["extract_prompt"] = " ".join(args[1:]).strip()
            save_config(cfg)
            print("extract_prompt задан у %s — цену будет извлекать модель.\n"
                  "Проверь: check --only %s" % (iid, iid))
            return OK
    return fail(E_NOTFOUND, "not_found", "нет товара с id: " + iid)
...
def main(argv):
    ...
    table = {"help": cmd_help, "doctor": cmd_doctor, "check": cmd_check, "add": cmd_add,
             "set": cmd_set, "prompt": cmd_prompt, "list": cmd_list, "methods": cmd_methods,
