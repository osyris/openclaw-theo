...
## Часовой пояс Наталии (Эстония)

**КРИТИЧЕСКИ ВАЖНО — проверять КАЖДЫЙ раз перед созданием напоминания:**
- **Зимнее время (EET):** UTC+2 — действует с последнего воскресенья октября до последнего воскресенья марта
- **Летнее время (EEST):** UTC+3 — действует с последнего воскресенья марта до последнего воскресенья октября
- Переход: в последнее воскресенье марта в 03:00 → 04:00 (на UTC+3), в последнее воскресенье октября в 04:00 → 03:00 (на UTC+2)

**Формула:** Наталия говорит "в 14:00" → вычисли UTC: `14:00 - offset`. Если сейчас зима → offset=2, если лето → offset=3.
...
### Правила от Наталии (25.03.2026):

1. **Вечерний чекпоинт** — ОДИН, 21:30 по Эстонии. Показывать ТОЛЬКО задачи текущего дня. Никаких будущих/прошлых.
2. **Проверка ДР** — 08:00 по Эстонии. Источники: birthdays.md + iCloud Calendar.
3. **Разовые напоминания** — создавать только по прямой просьбе Наталии. Не создавать лишних.
4. **Чекпоинт НЕ должен дублироваться** — только один вечерний чекпоинт!
5. **Всё время — Europe/Tallinn** в cron (не UTC offset).
6. **Информация в чекпоинте:** календарь на сегодня, сработавшие напоминания, Tele2 дедлайны сегодня, задачи на сегодня.
...
1. Получить рецепт от Наталии — скриншот, ссылка на сайт или текст
2. Извлечь все поля (название, ингредиенты, шаги, время, порции, категория)
3. **Фото:** попросить Наталию сгенерировать через ChatGPT или использовать красивое фото из интернета. Сохранить в  (slug = транслит названия через дефис, только латиница и цифры)
4. Добавить запись в  строго по шаблону ниже
5. Подтвердить добавление — дать ссылку на страничку

### Шаблон записи (все поля обязательны)
...
|---|---|
| breakfast | Завтраки, оладьи, каши |
| lunch | Обеды, супы, лёгкие блюда |
| dinner | Ужины, основные блюда |
| salad | Салаты |
| soup | Супы |
| snack | Закуски, перекусы |
| dessert | Десерты, выпечка, сладкое |
| drink | Напитки, смузи |
...
Теги свободные — добавляй по смыслу, не больше 3-4 на рецепт.

### Slug для имени файла фото

Правила: транслит русского названия → строчные латинские буквы → пробелы заменить на  → убрать всё лишнее.
...
### Источник рецепта (если ссылка)

Если Наталия дала ссылку — извлечь данные со страницы через инструмент чтения URL. Брать ингредиенты и шаги с сайта, а не придумывать.

### Обновление существующего рецепта

Найти запись по полю  в , обновить нужные поля, сохранить файл.
...
**База данных:** `/data/workspace/pages/recipes/recipes.json`
**Фото:** `/data/workspace/pages/recipes/img/`
**URL:** `nataliakatsalukha.com/pages/recipes/`

### Как добавить рецепт

1. Получить рецепт от Наталии — скриншот, ссылка на сайт или текст
2. Если ссылка — извлечь данные со страницы через инструмент чтения URL
3. **Фото:** Наталия генерирует через ChatGPT и присылает, либо найти красивое фото в интернете. Сохранить в `img/<slug>.jpg`
4. Добавить запись в `recipes.json` строго по шаблону ниже
5. Подтвердить добавление — дать ссылку nataliakatsalukha.com/pages/recipes/
...
```json
{
  ...
  "category": "breakfast|lunch|dinner|salad|soup|snack|dessert|drink",
  "tags": ["постное", "быстро", "для гостей"],
  "time": "30 мин",
  "servings": 4,
...
`постное`, `без глютена`, `для гостей`, `быстро`, `заморозка`, `пп`, `веганское`, `детское`, `праздничное`, `без молока`

Теги свободные — по смыслу, не больше 3-4 на рецепт.

### Slug для имени файла фото

Транслит русского названия → строчные латинские буквы → пробелы через дефис → убрать лишнее.

Примеры:
- "Куриные котлеты" → `kurinye-kotlety.jpg`
- "Пирог с ревенем" → `pirog-reven.jpg`
- "Мандариновый тирамису" → `mandarinovyy-tiramisu.jpg`

### Обновление существующего рецепта

Найти запись по полю `title` в `recipes.json`, обновить нужные поля, сохранить файл.
